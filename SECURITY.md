...
On the landing page, nothing she does leaves your browser tab. The page writes text, never markup. A strict CSP allows no outside scripts and no `eval`. `/api/ask` and `/api/pick` take JSON from this site only, at 20 calls a minute per visitor. The reader model answers from the article in front of it or declines. Every reply is capped, so no upstream can flood the chat.

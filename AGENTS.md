...
not a version bump of the last one, same pattern as Codex's model names
(Haiku, Opus, Sonnet, Fable), not "Codex-2/Codex-3". First: Samantha
...
- `.venv/` holds mlx-lm. `python3 training/prep_data.py` rebuilds `data/train.jsonl`
  ...
  AGENTS.md files. Never commit `data/` or `*-adapter/`, they're gitignored

...
## File size
No god files. Past about 500 lines a file gets split by what it does, with the old module re-exporting the moved names so nothing that imports it breaks. Targets now: tools.py (751, picker and agent in tools_agent.py), tools_util.py (651), pixelmator/pxm.py, web/demo.js, web/samantha.js. Law 8 in eval/laws.py enforces a ceiling (MAX_LINES) on every Python file, CI included; lower it after each split, never raise it. One split per loop round, each behind the full check set.
...
## Releases
Releases cut themselves. Bump `VERSION` (patch for a fix, minor for a new ability, major when a whole roadmap family or phase completes: the files family's write half is 4.0.0), run `python3 stats.py` to refresh `web/stats.json`, and commit as `Release vX.Y.Z: what shipped`. When that lands on main and CI passes, `.github/workflows/release.yml` tags it and publishes the GitHub release. Nobody runs anything by hand. `./release.sh X.Y.Z "what shipped"` on the Mac still adds the full gate (her real model, Pixelmator, the live page) and the site deploy, whenever the Mac is used. Never leave a day's work untagged: on 2026-09-21 76 commits piled up past v0.7.4 before anyone noticed.
...
Logos: `make_logo` always makes an icon, never text. Three modes: golden spiral (the default; her model picks palette, cell count, shape and how many glow), complex (say "complex"), and simple (say "simple" or "minimal": ring, spark, bars or dot). The spiral has two styles: spiral (open cells) and flower (packed petals round one bright core, reads at 16px). The icon (`web/icon.svg`, `icon.svg`, `web/samantha-logo.png`) is her flower: `tools_logo.icon_svg()` rebuilds it byte for byte and `test_edges.py` fails if anyone hand-edits it; the Pixelmator spec is `pixelmator/examples/turing-flower.json`. `tools_logo.layers_to_svg` draws any of her designs without Pixelmator. `docs/icon-blueprint.svg` is generated by `tools_logo.icon_blueprint_svg()` and pinned by a test, as are the measured rules: mark 70 to 80 percent of the tile, dark ring at least a pixel at 16px, core and petals at least 7:1 on the tile. Her mark (the woman in the engraved oval, `art/mark.svg`) is redrawn fresh at every release: `tools_logo.redraw_mark()` asks the hosted draw endpoint for a new woman-with-a-mind portrait and traces it to a candidate, `art/mark-candidate.svg`; `tools_logo.promote_mark()` only ships it once `mark_sanity()` passes a plain pixel check (ink fraction, a clean margin, no stray blot, actually different from what's shipped). Both are wired into `release.sh` before the version bump, and either one giving up (a rate limit, a failed check) just keeps the current mark and prints why, never failing the release.

...
## Usage
The loop watches Claude usage every round: the session's `rate_limit_info` (from `get_session`) and its running cost. While status is allowed and there is no overage, a round every 15 minutes or so. On overage or any status other than allowed (`allowed_warning` included: that is usage nearing its limit), stop new work: no new builds, only CI checks, red fixes and merging PRs already open, until the window resets. Say so in one line when stopping.

## Branches
Work reaches main through a PR from the session's branch, and the loop merges it into main itself as soon as every check passes; Joshua never has to handle a PR. Before every push, run the full CI check set locally (every test file, `tools.py`, `tools_util.py`, eval/actions.py, eval/web_parity.py, eval/util_diff.py, eval/laws.py, `stats.py --check`); push only when all pass. After the push, check every workflow run (test, release, deploy) and fix any red at once.

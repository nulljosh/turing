...
## File size
No god files. Past about 500 lines a file gets split by what it does, with the old module re-exporting the moved names so nothing that imports it breaks. Targets now: tools.py (778, agent moved to tools_agent.py), tools_util.py (651), pixelmator/pxm.py, web/demo.js, web/samantha.js. Law 8 in eval/laws.py enforces a ceiling (MAX_LINES) on every Python file, CI included; lower it after each split, never raise it. One split per loop round, each behind the full check set.

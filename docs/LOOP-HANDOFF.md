...
2. 5.0, her own head (round nine shipped 1300/1895, 83 wrong past guard; bar is under ten). Picker rounds stall when the builder hands back mid-eval: brief it to poll in the foreground. Next round: attack the remaining wrong-past-guard clusters in BAKEOFF.md round nine. retrain the picker on the templates plus training/feedback_to_data.py rows, score with eval/hands.py against round six. Check free memory first, never two mlx_lm.lora jobs at once.

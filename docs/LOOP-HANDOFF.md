...
## Next, in order
1. Round three of the picker is trained (2026-09-21): 394/501 unseen, 73/77 actions, 93/94 questions, and it knows `paint_image`. 21 wrong picks still get past the guard in `tools.do()`. Tighten `_sound()` next. The adapter is gitignored and vanished from disk once already, so back it up to the LaCie.

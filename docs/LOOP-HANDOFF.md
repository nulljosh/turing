...
2. 5.0 check, done honestly: build a fresh held-out picker test set (new phrasings by a different writer, e.g. the local 9B or a Sonnet subagent that never saw hands-data/ or the guard rules, filtered against every existing test and training row), score the shipped adapter plus guard on it. 5.0 ships only if that fresh set also shows under ten wrong past the guard and zero refused. Known leaks: "swing by X", "head on over to X" still open_app.

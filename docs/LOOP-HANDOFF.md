...
## What the loop is
Samantha to a 1.0.0 release, landing page to A+, and a standing gap hunt. Each iteration: look at what comparable assistants do that she cannot (Siri and Shortcuts, Apple Intelligence, Claude and ChatGPT desktop with MCP, Open Interpreter, Raycast AI, local Ollama agents), add the biggest real gap to `roadmap.md` under "Gaps found by the loop" with where it was seen, then build the smallest honest fix and delete the line once it ships. Every fix ships with its tests, `eval/laws.py` green, docs at 100 percent, the phone-size QA on the landing page, and CI green before the next iteration. `roadmap.md` "Road to 1.0.0" is the floor, the gap list in the same file is the ceiling. After every ship: prune the roadmap, refresh the landing page, README and CLAUDE.md, deploy.

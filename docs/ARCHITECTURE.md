...
| `tools_util.py` | Thirty-nine utility tools that need no app: calculate, convert_units, time_in, current_date, days_until, flip_coin, roll_dice, random_number, make_password, make_uuid, hash_text, base64_encode, base64_decode, word_count, reverse_text, shout, morse_code, json_pretty, is_prime, roman_numeral, tip, plus the Mac readers disk_space, uptime, memory_usage, cpu_load, ip_address, wifi_name, system_info and the three that act, copy_to_clipboard, sleep_display, reveal_in_finder, list_shortcuts, run_shortcut the MCP client pair list_mcp_tools and call_mcp_tool, and the Chrome tab tools list_tabs, switch_tab, close_tab and read_tab (close_tab asks first and is never a model's choice) (run_shortcut and call_mcp_tool only when named, never chosen by a model; call_mcp_tool asks first). Fixed argv only, silent under SAMANTHA_HEADLESS. Its ROUTES table is appended to the router in tools.py. |

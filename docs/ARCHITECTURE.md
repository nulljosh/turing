...
| `tools_util.py` | Forty-five utility tools that need no app: calculate, convert_units, time_in, current_date, days_until, flip_coin, roll_dice, random_number, make_password, make_uuid, hash_text, base64_encode, base64_decode, word_count, reverse_text, shout, morse_code, json_pretty, is_prime, roman_numeral, tip, plus the Mac readers disk_space, uptime, memory_usage, cpu_load, ip_address, wifi_name, system_info and the three that act, copy_to_clipboard, sleep_display, reveal_in_finder, list_shortcuts, run_shortcut the MCP client pair list_mcp_tools and call_mcp_tool, the Chrome tab tools list_tabs, switch_tab, close_tab and read_tab, memory across sessions with remember, recall and forget (a JSON file in ~/.samantha, private: hidden from models and MCP, remember and forget ask first), read_document and find_in_document (PDF through pdf.swift and PDFKit, Word, RTF and HTML through textutil, plain text, home folder only), read_screen (macOS Vision OCR through ocr.swift on a fresh screenshot, private and ask-first), and close_tab asks first and is never a model's choice (run_shortcut and call_mcp_tool only when named, never chosen by a model; call_mcp_tool asks first). Fixed argv only, silent under SAMANTHA_HEADLESS. Its ROUTES table is appended to the router in tools.py. |

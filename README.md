...
Once the pipeline is boring and repeatable, try a 1.5B-3B base (Qwen2.5-3B, Llama-3.2-3B). Bigger model = slower training, more memory, better baseline fluency. Compare quality per minute of training against the 0.5B — there's a real chance the 0.5B fine-tuned on great data beats a 3B fine-tuned on so-so data.

...
- **2026-09-13, Phase 4 attempt, blocked:** built `ask.py` (retrieval-augmented generation via `brain`'s `/api/search`) and expanded `brain/ingest.mjs` to pick up `WHITEPAPER.md`/`FAQ.md`/`TROUBLESHOOTING.md`, not just README/roadmap/CLAUDE.md. Re-ingest reported `{"ok":true,"count":N}` for all 21 batches (1033 chunks). But search for the exact literal opening sentence of `FAQ.md`, byte-for-byte, returns zero hits after 90+ seconds of polling, Turing's content genuinely is not queryable despite the server confirming it accepted the upload. `src/tools.ts`'s `ingest()`/`retrieve()` code looks correct on inspection, single `VEC` binding in `wrangler.jsonc`, no obvious dev/prod split. This is a real bug in `brain`'s Vectorize pipeline, not diagnosable further without Cloudflare dashboard/deploy logs access. Phase 4 is blocked on this, not done, `ask.py` is real and correct code, ready to work once the indexing bug is found.

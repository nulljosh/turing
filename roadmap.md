...
- [x] Image tools on ImageMagick, Pixelmator retired from every gate. Direct request, 2026-09-23
...
- [x] Research, the rest: a page named in the question (a URL, or a site when Wikipedia has none) is fetched and cited alongside Wikipedia, tags stripped by the same reader read_page uses; "tell me more about X" tries the last brief's own sources before searching again; a JS-only or blocked page is declined honestly, never silently dropped. save_research (writing to a file, asking first) had already shipped. 2026-09-24

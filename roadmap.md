...
- **2026-09-13, run 4:** oversampled own docs 10x (only 17 unique chunks) on top of run 3's weights, 500 more iters. Overfit hard, train loss 0.5-0.6 vs val loss 1.26, output degraded into verbatim doc parroting and garbled tokens regardless of prompt. Real lesson: neither more repeats of a small pool nor more iterations substitute for more distinct real content. `OWN_REPEATS` dropped 10→3, next run trains fresh from base (not resumed on top of the overfit weights) to avoid compounding the damage.

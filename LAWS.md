...
12. A model-driven loop (`tools_agent.agent`, `tools_screen_agent.screen_task`, planner's model path) still checks itself before a WRITE it proposed reaches the human confirm: `intent.check()` refuses a step whose tool and argument do not trace back to the user's own request, whatever a reading tool's result talked it into proposing. A direct command the router matched on the user's own words is never double-checked this way.
